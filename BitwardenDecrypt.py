...
def decryptBitwardenJSON(options):
    ...
    try:
        totestPassword = options.password
    except Exception as e:
        print(f"ERROR: An error occured assigning the password: {options.password}")
        sys.exit(1)

    getBitwardenSecrets(datafile["userEmail"], \
        totestPassword.encode("utf-8"), \
...
def main(options):
    ...
    print("SUCCESS decrypting the data.JSON file!!!! Writing the cleartext passwords into the file Cleartext.json")
    file = open("Cleartext.json", "w")
    str_dictionary = repr(decryptedJSON)
...
if __name__ == "__main__":
    ...
    parser.add_argument("password", nargs='?', default="password", help='PASSWORD (optional)')
